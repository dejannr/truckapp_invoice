    ...
    <main className="min-h-screen grid place-items-center p-6">
      <div className="card w-full max-w-lg">
        <div className="card-body space-y-4">
          ...
          <button className="btn btn-primary w-full" onClick={async () => { try { const res = await api('/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) }); localStorage.setItem('token', res.accessToken); if (form.companyName) localStorage.setItem('companyName', form.companyName); router.push('/dashboard'); } catch (e: any) { setError(e.message || 'Registration failed'); } }}>Create Account</button>
