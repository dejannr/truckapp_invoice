  ...
  return <AppShell title="Invoice Detail">
    {!invoice ? <div className="skeleton h-60" /> : <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
      ...
      <div className="space-y-4">
        <div className="card"><div className="card-header"><h3 className="font-semibold">Actions</h3></div><div className="card-body space-y-2"><a className="btn btn-primary w-full text-center" href={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api'}/invoices/${id}/download`} target="_blank">Download PDF</a><button className="btn btn-secondary w-full">Mark as Sent</button><Link href={`/invoices/${id}/review`} className="btn btn-secondary w-full text-center">Edit Data</Link><Link href="/invoices" className="btn btn-secondary w-full text-center">Back to Invoices</Link></div></div>
      </div>
    </div>}
  </AppShell>;
}
